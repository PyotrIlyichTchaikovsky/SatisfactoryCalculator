...
const localVersion = process.env.RELEASE_VERSION || 'v2000.01.01.0.0';
...
module.exports = defineConfig({
  ...
  webServer: remote ? undefined : [
    {command:'python -m uvicorn production_planner_app:app --app-dir satisfactory_calculator/recipe_web --host 127.0.0.1 --port 8766',url:'http://127.0.0.1:8766/api/health',reuseExistingServer:false,
      env:{SENTRY_DSN:'',SENTRY_ENVIRONMENT:'staging',SENTRY_RELEASE:sha,PLANNER_VERSION:localVersion,PLANNER_ALLOWED_ORIGINS:'http://127.0.0.1:8788'}},
    {command:'python scripts/preview_frontend.py',url:'http://127.0.0.1:8788/',reuseExistingServer:false,
      env:{SENTRY_DSN:'',SENTRY_BROWSER_SCRIPT_URL:'',SENTRY_ENVIRONMENT:'staging',SENTRY_RELEASE:sha,RELEASE_VERSION:localVersion,RELEASE_ID:'local-check',PLANNER_API_BASE_URL:'http://127.0.0.1:8766',PUBLIC_SITE_URL:'http://127.0.0.1:8788',ADSENSE_ENABLED:'false',FRONTEND_OUTPUT_DIR:'dist/frontend'}}
  ]
});
